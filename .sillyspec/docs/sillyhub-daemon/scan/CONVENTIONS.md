...
6. **接口优先于具体类做依赖**：`task-runner.ts` 用 `RunnerHubClient` / `RunnerWorkspaceManager` / `RunnerCredentialManager` 鸭子类型接口（避免硬耦合具体类）；`daemon.ts` 用 `RecoveryCoordinator` / `DaemonOptions` / `InteractiveCredentialManager` 接口。
...
10. **CLI 子命令拆独立 action 函数**：`startAction` / `stopAction` / `statusAction` / `logsAction` 各自导出，便于单元测试直接调用。
...
- **命名**：
  ...
  - 私有成员：`_` 前缀（`_ws` / `_handleMessage` / `_routeSessionControl` / `_persistence` / `_recoveryClient`）。
...
- **安全边界**：`file-rpc.ts` 强制 `allowed_roots` 校验（`assertWithinAllowedRoots`）；`task-runner.ts` 防 tar 路径穿越；`credential.ts` 占位符仅全串匹配不做子串替换（`{{USER_XXX}}`）。
...
- **超时/重试参数集中为常量**：`permission-resolver.ts` `PERMISSION_FALLBACK_TIMEOUT_MS`、`ws-client.ts` `RECONNECT_INTERVAL_MS` 等。

## 历史映射约定（源码残留注释）

源码中大量注释带"对齐 Python `xxx`"的映射说明（如 `config.ts` 的 `等价 Python DEFAULT_CONFIG_DIR`、`hub-client.ts` 的 `对齐 Python httpx.AsyncClient`）。这些是 task-21 Python→TS 重写时留下的对照注释，**不是当前技术栈**，可在后续清理债务时移除（见 CONCERNS.md）。
