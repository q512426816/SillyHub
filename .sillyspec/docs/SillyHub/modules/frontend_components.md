...
- 会话域（daemon/ + sessions/，两处共享子组件）：
  ...
  - `session-usage-bar.tsx` — 会话用量条（2026-08-29-session-usage-stats）：摘要行五指标+缓存命中率（cache_read÷(cache_read+input)，分母 0「—」）+按模型折叠明细；自取数（useEffect+refreshSignal prop，零 react-query 对齐 dialog 渲染约束），session-panel page（头部下方）/dialog（输入框上方）双模式挂载，轮次终态递增信号重取
