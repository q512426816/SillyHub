...
- 取数 hooks：
  ...
  - `use-daemon-machines` — 机器级列表，refetchInterval 15s（ql-20260921-009-b4f4：opts.refetchInterval 可覆盖——runtimes 页升级 sillyspec 下发后 60s 窗内传 5s 加速拉取横幅回报，选项不进 queryKey、值变化 react-query 重排轮询定时器）；sessions（100 行级重列表）默认不拉、opts.includeSessions 才并发（ql-20260909-013-5c88：其余挂载方 15s 白拉清零；ql-20260910-002：门户/悬浮宿主消费 sessions（继续最近会话/D-005 回退），拆分时漏传致入口消失——已补传 true，消费方=机器页+门户+悬浮宿主）——includeSessions 进 queryKey（daemonMachinesQueryKey 导出 helper，setQueryData 侧写必须同 key）
...
- ql-20260921-009-b4f4 | use-daemon-machines 增 opts.refetchInterval 覆盖口（缺省 15000 原语义零变化）——runtimes 页升级 sillyspec 回显提速第二级（镜像 ql-20260911-024 platform-sync 加速窗）：下发后 60s 窗内机器查询切 5s 短间隔拉 sillyspec_update 回报，窗外回退 15s；选项不进 queryKey（不影响缓存身份），值变化由 react-query 重排轮询定时器。daemon 面同 ql 条目见 daemon.changelog。
