...
SillyHub 产品根本身**没有任何测试**（根 `package.json` 的 `test` 是默认 `echo "Error: no test specified" && exit 1` 占位）。
所有测试都在 3 个子项目内独立运行，**命令必须在对应子项目目录下执行**，或通过根 `Makefile` 的聚合 target。
backend 测试覆盖率门槛 ≥ 60%（`make backend-test` 文档约定）。
...
| 子项目 | 框架 | 测试文件数 | 运行命令 |
| --- | --- | --- | --- |
| backend | pytest + pytest-asyncio | 150 | `cd backend && uv run pytest` |
| frontend | vitest + Testing Library | 33 | `cd frontend && pnpm test` |
...
- backend dev 依赖用 `aiosqlite`（async sqlite）跑测试，生产用 `asyncpg` + PostgreSQL，存在 async 驱动方言差异风险（见 CONCERNS.md）。
