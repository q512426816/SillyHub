...
- **interactive kill 是假停(僵尸 bug)**:用户点"终止"界面立即显示已停,但 daemon 里 claude/codex 进程继续跑、继续烧 token。根因 `backend/app/modules/daemon/lease_service.py:435-448` `_ws_cancel_stub` 只打一行日志不发 WS,而 `DaemonWsHub.send_session_control`(SESSION_INTERRUPT/END)早已就绪。来源 `docs/agent-platform-deep-audit-2026-07-12.md` 发现 1(P0-1)。
- **MissionControl.cancel 造僵尸**:`backend/app/modules/agent/control.py:83-99` cancel mission 时只改 `AgentRun.status`,不调 `cancel_lease`,daemon 不被通知、worker 继续跑(同上 P0-2)。
- **scan 文档全量结构性过期**:5 套 scan 都停在 `source_commit ba87eec`,跨过 daemon-entity-binding 重构,影响归档/影响分析/模块边界判断(`ROADMAP.md` 第四节 🔴 P0)。
...
- **写代码 team mission 断 2 处 + 共享 worktree 硬阻塞**:`finalize_execute_mission`(`backend/app/modules/agent/finalizer.py:167-186`)全代码无调用点;`collect_completed_artifacts`(`execution.py:168`)没把 patch 存成 `AgentArtifact(kind='patch')`;`dispatch_worker`(`execution.py:104-130`)给每个 worker 传同一 `root_path`,多个 impl worker 并行写会互相覆盖文件 + patch 基线漂移(审计发现 3 / P2-2,D-006 已知延后项)。
...
- **migration 链断裂**:SQLite 单测验不到 PG 方言差异与多 head;部署前必跑 `alembic heads` 核实单头(当前 `202607251000`),并行变更各写 migration 易撞 revision/down 分叉;子代理手算 revision 图漏 merge revision 会误报多 head,以官方 `alembic heads` 为准。
