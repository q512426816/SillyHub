...
- LLM 调用经 LiteLLM 网关统一出口：backend `llm_provider` 模块持 `litellm_client.py`；daemon 侧经 backend `/api/llm-proxy/{path}` 透传端点（`backend/app/modules/daemon/router.py:4765`，master key 不出 backend 进程）。
