...
- 流转: `transitionChange(workspaceId, changeId, targetStage, reason?, provider?)`
- 执行: `executeChange(workspaceId, changeKey, provider?)` — 创建 AgentRun 并后台执行（2026-06-14 起 provider 覆盖默认 agent）
- 反馈: `submitFeedback(workspaceId, changeId, category, text)`
- 归档门禁: `checkArchiveGate(workspaceId, changeId)`
- Agent 状态: `getAgentStatus`、`triggerDispatch(...provider?)` — 手动 dispatch 覆盖默认 agent
...
<!-- MANUAL_NOTES_START -->
- 2026-06-14-agent-runtime-selection：`transitionChange` / `executeChange` / `triggerDispatch` 均增可选 `provider?` 参数（POST body/query 透传），覆盖 workspace 默认 agent。
<!-- MANUAL_NOTES_END -->
