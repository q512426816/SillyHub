...
- plan-nodes 展开行内子表（明细 `PpmSubTable` editable / 模块 AntD `Table`）外层各套限宽 `calc(100vw - 340px)` + `overflowX:auto` 容器，隔离母表横向滚动（避免子表 `scroll.x:max-content` 撑长母表）；明细 7 列列宽紧凑（合计 790px）。仅 plan-nodes 本地，未改 `PpmSubTable` 通用组件。
