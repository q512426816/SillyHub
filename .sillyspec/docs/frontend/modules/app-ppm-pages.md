...
- `KanbanPage`（`/ppm/kanban`）：任务看板，数据全部来自 `useKanbanStore`（tasks 含 user_id + deadline），按 filters 做分组/筛选，用户列表由 store 的 `fetchUsers` 提供。点卡片展开 `KanbanTaskDetailDrawer` 详情抽屉，Tabs = 执行记录 / 评论 / 附件（执行记录复用 `lib/ppm/task.ts` 的 `listTaskExecutes({plan_task_id})`，只读 4 列表，对齐任务计划/问题清单详情）。
...
- plan-nodes 展开行内子表（明细 `PpmSubTable` editable / 模块 AntD `Table`）`scroll.x` 用**固定宽度**（明细 790 = 列总 710 + 操作 80；模块 790，模块名列自适应剩余），**不用 `max-content`**（ql-008 改；避免 antd 嵌套 max-content 测量膨胀撑长母表）。明细 7 列列宽压缩（合计 790px）。2K 屏 790 < 可视宽度 → 母表不撑无滚动条、子表 fits 不滚；窄屏子表按 790 独立滚不撑母表。曾试限宽 overflow 容器（ql-003/005）2K 屏引入多余滚动条，ql-007 已回退。仅 plan-nodes 本地，未改 `PpmSubTable` 通用组件。
