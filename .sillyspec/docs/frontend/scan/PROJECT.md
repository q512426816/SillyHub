...
10. 当 Agent 需要用户指导（pending_input）时，在日志下方展示交互输入面板，用户可提交指导文本
...
6. 识别 `pending_input` 通道日志时，展示交互输入面板，用户可提交指导文本
7. 提交指导通过 `POST /agent/runs/{run_id}/input` 接口，成功后 SSE 推送 `user_input` 事件
8. 对已完成 run 的展开日志，`pending_input` 和 `user_input` 以只读标记样式展示
