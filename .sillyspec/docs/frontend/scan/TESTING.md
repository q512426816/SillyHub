...
- 类型逃逸口受控：全 src `@ts-ignore` 0 处，`@ts-expect-error` 仅 1 处（`frontend/app/api/daemon/sessions/[sessionId]frontend/app/api/agent/runs/[runId]frontend/app/api/agent/runs/[runId]/stream/route.ts:63`，undici compress 非标准 RequestInit 字段的注释性豁免）。
