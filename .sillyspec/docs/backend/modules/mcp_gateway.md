...
- 4 个 change 阶层 `@mcp.tool()`（tools.py，task-07/08/09/10，design §6.1 / D-004，形态A 按需触发替代被砍的 auto_dispatch）：
  ...
  - `submit_stage_review`(dispatch)：包装 review 四方法（proposal_review/plan_review/human_test/archive_confirm），前置 `StageProjectionService.compute_pending_review` 校验。2026-08-14（D-004/D-006@v2）：审批**不派发 agent**（返回 `agent_dispatch` 恒空，`dispatched: False`）；新增 `notify_session` 参数（缺省 true）——审批落库后以服务身份向绑定会话注入消息，响应 `notified_session` / `notify_error`（对齐 HTTP ReviewResponse）。
