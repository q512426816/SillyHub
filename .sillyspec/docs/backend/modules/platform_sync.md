...
- 变更事件通道（2026-09-23-change-events-channel，watcher 旁路观测面，**provisional 只展示不消费红线**）：
  - `POST /api/changes/{name}/events`（写）：CLI watcher 批量上行（`{events:[...]}` 1..200 条，kind 必填/ts epoch 毫秒 `[1e12, 253402300799999]` 值域（ql-20260924-001 M-2 补上界 + 非有限值 before 校验器转 None——防 Infinity/微秒误传在落库处 Overflow/OSError 整批 500，且回显 inf 的 422 响应自身因 starlette `allow_nan=False` 渲染炸 500）/stage·detail·rule·severity·id 可选，extra=ignore）；`(workspace_id, change_name, dedup_key)` 幂等**跳过不覆盖**（dedup_key：事件 id 优先否则 `ts|kind|stage` 拼串）；INSERT+修剪+commit 包 IntegrityError 重试收敛（ql-20260924-001 M-1——并发对手在本批预取后提交同 dedup_key 时 rollback 重查剔除撞键条目重插，对齐 upsert :535 先例，不再整批 500）；`provisional` 落库恒 True（请求值丢弃）；单变更 >5000 条同事务删最旧修剪（`PlatformChangeEventORM` / `platform_change_events` 表）。
