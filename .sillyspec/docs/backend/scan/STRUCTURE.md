...
> 范围：`backend/`（不含 `.venv/`、`.pytest_cache/`、`__pycache__/`、`uv.lock`）。基于 `ls`/`grep` 实测于 commit `ba87eec`。
...
│   ├── main.py                 # create_app() 入口、lifespan、中间件、路由挂载、quick-chat 内联路由（qc_router）
│   ├── core/                   # 横切关注点（13 个 .py，见下表）
│   ├── models/
│   │   └── base.py             # BaseModel(SQLModel) 公共基类
│   └── modules/                # 26 个业务目录 + ppm 子域（见下表）
├── hooks/
│   └── scan_write_guard.py     # 扫描文档写入守卫钩子
├── migrations/
│   ├── env.py                  # Alembic 异步运行环境
│   └── versions/               # 63 个 revision（含 2 个 merge head），命名 YYYYMMDDHHMM_<desc>.py
├── scripts/                    # 辅助脚本
└── tests/                      # 顶层集成测试（与 app/modules/*/tests/ 并存，均被 pytest 收集）
    ├── test_config.py
    ├── test_health.py
    ├── core/                   # test_auth_deps_principal.py、test_config_spec_transport.py
    └── modules/                # admin/ agent/ auth/ change/ change_writer/ daemon/ workspace/
```
...
| 文件 | 职责 |
| --- | --- |
| `config.py` | `Settings(BaseSettings)` + `get_settings()`（`@lru_cache` 单例），双层 env / `.env` 覆盖；`resolve_spec_data_root` 校验器 |
| `db.py` | 进程级 `AsyncEngine`（`create_async_engine`）+ `async_sessionmaker` + `get_session` 依赖 + `get_session_factory()` + `dispose_engine` |
...
## `app/modules/` 目录职责（26 个业务目录 + ppm 子域）
...
| `workspace` | 项目工作区 + 多对多关系图 + 成员管理 | `model.py`、`router.py`、`members_router.py`、`members_service.py`、`relation_service.py`、`parser.py`（旧） |
| `change` | SillySpec 变更主实体 + 状态机 + dispatch + 9 个 prompt 模板 | `model.py`、`router.py`、`service.py`、`dispatch.py`、`parser.py`、`prompts/{brainstorm,propose,plan,execute,verify,scan,quick,clarifying,archive}.md` |
| `task` | 变更下的任务 | `model.py`、`router.py`、`service.py` |
| `change_writer` | 变更文档落盘 | `service.py`、`router.py`、`schema.py` |
| `workflow` | 变更评审 + 审计日志 + 旧 FSM（已 deprecated）+ spec_guardian | `model.py`、`router.py`、`service.py`、`fsm.py`、`spec_guardian.py` |
| `agent` | Agent 运行编排：协调器/mission/execution/delegation/diff收集/post-scan 校验/context builder/control；`adapters/` 目录当前仅含 `__init__.py`（空） | `model.py`、`service.py`、`router.py`、`coordinator.py`、`placement.py`、`base.py`、`mission.py`、`execution.py`、`delegation.py`、`diff_collector.py`、`post_scan_validator.py`、`context_builder.py`、`control.py`、`adapters/` |
...
| `git_identity` | Git 身份管理（含 GitHub OAuth provider） | `model.py`、`router.py`、`service.py`、`providers/{base,github}.py` |
| `tool_gateway` | 工具调用网关 + 工具策略 + 操作日志 | `model.py`、`router.py`、`policy_router.py`、`policy_schema.py`、`tool_policy`(逻辑) |
...
| `spec_profile` | spec profile（仅 model/schema，router/service **未实现**） | `model.py`、`schema.py` |
...
| `ppm`（子域） | 项目/计划/任务/问题/看板，统一 `/api/ppm` 前缀；common 为共享 | `project/`、`plan/`、`task/`、`problem/`、`kanban/`、`common/`（各含 model/router/service/schema） |

> 注：`main.py` 内联 `qc_router`（`/api/daemon-chat*` 一组 quick-chat 端点：POST 创建、GET 结果、GET stream(SSE)、POST kill、GET logs），不走 modules 目录。

## 按文件类型统计

- `router.py`：27 个（含 PPM 子域 5 个、workspace members_router、tool_gateway policy_router；不含 `main.py` 内联 `qc_router`）
- `service.py`：30 个（含 daemon 5 子包 service + facade、workspace members/relation service、admin 3 service）
- `model.py`：23 个定义 ORM 模型的文件（ppm common 无 model）
- `table=True` SQLModel 表：66 张（grep `table\s*=\s*True` 命中，含子表/关联表）
- Alembic 迁移：63 个 revision 文件（含 2 个 merge head：`merge_orchestration_and_ppm_heads`、`merge_heads`）
- prompt 模板：9 个（`change/prompts/`）
