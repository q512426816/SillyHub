...
implementation:
  ...
  - 创建 AgentRun 照 bootstrap 先例（spec_workspace/bootstrap.py:128-150）——status=pending、agent_type=claude_code、provider/model 取 workspace.default_agent/default_model 兜底，metadata_ ORM 属性（agent/model.py:412，DB 列名 metadata）写 kind=knowledge-distill 与 source_type/source_ref/focus 四键，建 AgentRunWorkspace(agent_run_id, workspace_id) 关联，后台执行任务持强引用防 GC（_BACKGROUND_BOOTSTRAP_TASKS 同款 holder）
  - 后台执行链照 bootstrap.py:456-476——RunPlacementService.decide_backend 抛 NoOnlineDaemonError 时立即置 status=failed、error_code=no_online_daemon、finished_at、exit_code=1、output_redacted 后 commit 并发 done 事件，任务创建本身成功且状态立即可查
