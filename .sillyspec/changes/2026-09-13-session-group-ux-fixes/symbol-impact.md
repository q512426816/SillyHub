...
- task-01: **签名级变更：有（可选参追加，零破坏）**。`readSessionDraft(sessionId)` / `writeSessionDraft(sessionId, draft)` / `sessionDraftLsKey(sessionId)`（frontend/src/components/daemon/session-panel/turn-state.ts:351）各追加可选第三参 `preScope?: string | null`。受影响调用点全仓 4 处：frontend/src/components/daemon/session-panel/session-panel-page.tsx:451/461（本任务改传 scope）、frontend/src/components/daemon/session-panel/session-panel-dialog.tsx:271/279（本任务改传 scope）——全部在任务 allowed_paths 范围内；无其他调用方（grep 全仓核验）。
