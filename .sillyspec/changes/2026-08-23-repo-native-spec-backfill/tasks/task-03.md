...
implementation:
  ...
  - 指针写入（command.js:364 writePlatformPointer 调用处）：门禁从裸判定改 isPlatformMode（自指 flag 不落盘）；init.js:423 isExternalSpec 词法 resolve 比较补 realpath（真目录同路径维持既有词法命中，junction 回环新增命中 → writeInitPlatformPointer return 不写）
