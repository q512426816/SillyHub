...
goal: >
  ...
  数据源（FR-02）。
implementation:
  - '单源加键：sillyhub-daemon/src/interactive/providers.ts 的 ProviderCaps 接口（:65）加 steering: boolean；PROVIDER_CAPS 表（:264）四引擎条目补取值 pi/claude/codex=true、cursor=false；docblock 补第 14 键取值依据——pi 为 _sendInject steer 通道实证（pi-rpc-driver.ts:1859-1914）、claude 为 query({prompt: AsyncIterable}) 忙轮推流经 SDK 命令队列吸收（claude-sdk-driver.ts:402，投递时机以 task-04 实测为准）、codex 为 app-server turn/steer 方法存在（参数以 task-02 实机探测为准）、cursor 无对应通道=false、未知 provider 默认 false'
