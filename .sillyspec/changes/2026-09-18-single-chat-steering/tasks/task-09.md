...
implementation:
  ...
  - 类型重生成：跑 pnpm gen:types 刷新 frontend/src/lib/api-types.ts（steered/dispatch_mode 等新出参入生成物），backend/openapi.json 随链刷新并一并提交（gen:types 链尾自动执行 gen-provider-caps）；核对 sessions.ts:274 手写镜像 steered 字段（task-07 手补）与生成物语义一致不漂移
