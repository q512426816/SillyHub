...
implementation:
  ...
  - 造数据 helper：经 db_session 直接 ORM 插入 User + AgentSession/AgentRun/AgentRunLog（app/modules/agent/model.py）+ AgentSessionTask（app/modules/daemon/model.py:662）+ SessionAttachment（app/modules/session_attachment/model.py），按 user_id 区分 owner/他人、软删置 deleted_at、群会话造参与者与非成员
  ...
  - 路由顺序：照 test_runtimes_usage_endpoint.py:148 先例双证——app 路由表断言 /sessions/export 字面量前置于 /sessions/{session_id} 参数路由，且真实 POST /api/daemon/sessions/export 不返回被参数路由吞掉的 422
acceptance:
