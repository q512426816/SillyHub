...
- [ ] task-11: 前端 /admin/users 页面 + 用户编辑 Drawer 组件（users/page.tsx 列表 + 搜索 + 筛选；admin-user-drawer.tsx 组织/角色多选 + 登录权限 + is_platform_admin 开关）

## Wave 8（端到端验证）

- [ ] task-12: 端到端验证（8 项关键路径：自保护/最后管理员/角色占用/组织占用/登录控制/会话撤销/审计覆盖/旧端点兼容；数据库空库+含数据双重迁移；alembic upgrade/downgrade 双向测试）
