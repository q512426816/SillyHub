...
implementation:
  ...
  - test_profile_service.py 新增 TestListVisibleAll，复用现有 _make_user/_make_workspace/_make_member/_make_profile helper，覆盖 actor A 不见 B private、非成员不见该 ws 的 workspace 级、owner-left-ws 后该档对其不可见（R-07）、聚合集含自己 private 加所属 ws 级加平台预置
...
acceptance:
  ...
  - 越权用例通过（A 不见 B private、非成员不见该 ws workspace 级、owner-left-ws 该档不可见）
