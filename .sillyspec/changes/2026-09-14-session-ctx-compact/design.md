...
- **claude**：SDK 官方支持 prompt 文本分发 slash 命令（sdk.d.ts AgentSpec 文档 + Agent SDK docs）——**backend 既有 inject 链路原样复用即可**（inject 服务原生建 run：response 带 run_id，turn 正常收敛，会话流天然出现 /compact 轮，与 CLI 原生体验一致）。压缩发生时下行 `compact_boundary` 系统帧（sdk.d.ts:3191-3213，带 `compact_metadata.pre_tokens/post_tokens/duration_ms`），现被 `claude-events.ts:605` 白名单外静默丢弃——**v1 不透传**（NG-04，呈现已由压缩轮本身承载）。
- **pi**：原生 RPC 命令 `{"type":"compact","customInstructions"?}`（pi 包 docs/rpc.md:374-411；rpc-mode.js:415-421）——**结构化回执**（response data 含 `tokensBefore/estimatedTokensAfter/summary`）+ `compaction_start/compaction_end` 事件。driver `_sendCommand`（pi-rpc-driver.ts:1732-1769）stdin 独立通道 + id→pending 关联 + stdout 监听常挂循环外（:1417）——**Grill X-b 实核：空闲态 consume 停在 inputIt.next()（:1444）时命令 response 照常 resolve，interrupt()/get_state 即同款先例，可行**。引擎的 compaction_* 事件维持现状吸收不透传（pi-events.ts:245-249）——回执走控制命令结果（见下）。
...
- **平台 RPC 通道（v3 定案）**：backend→daemon 的请求-结果机器**实存且先例干净**——`ws_hub.send_rpc(daemon_id, method, params, timeout)`（ws_hub.py:502-560：发 daemon:rpc 等 daemon:rpc_result，DaemonRuntimeOffline/DaemonRpcTimeout/DaemonRpcRemoteError 异常全集 + 动态默认超时）；daemon 侧 `ws.registerRpcHandler(method, handler)` 注册表（daemon.ts:6438-6456：list_dir/list_roots/get_spec_bundle/host_fs.stat 四先例，handler 返回对象即 RPC result）。**压缩走该通道**：backend 端点 send_rpc(daemon_id, 'session_compact', {session_id}, timeout=15) → daemon 注册 session_compact handler → sessionManager.compact → CompactResult 即 RPC result。v2 曾设想的 SESSION_COMPACT 控制命令机器被复审证伪（ack 全链 ids-only、表无 result 列、handlers 无返回值通道），全部弃用。
- **呈现链关键事实（Grill X-f）**：task_notification 瞬时通道在 daemon 消费侧被三重拦截（background-tasks.ts:201-202/:211-219/:227-232）+ 误触发唤醒注入（:253-261）；前端瞬时 SSE 不渲染为会话流行（session-stream.ts:318）、[SYSTEM:] 落库行被 assembler 丢弃（session-log-assembler.ts:297）——**会话流系统提示行路线不可行，呈现改为端点响应回执**。
- **前端挂点**：CtxUsageRing Popover（ctx-usage-bar.tsx:204-239）编辑器行后插按钮行；可选回调 `onCompact?` 模式（照 onWindowOverrideChange :120）；caps 门控参照（:439）；预会话态（session-panel-page.tsx:2722）无 sessionId 不渲染；turn running 派生现成（`running = turnState.currentRunId != null` :1451）。环分子 `latestCtxTokens` 逆序最新非 null（page-helpers.tsx:521）——**压缩后下一次调用 usage 到达即自然回落，零改动**。
...
- **旧 daemon**：未注册 'session_compact' handler → RPC 远端错误 → DaemonRpcRemoteError 映射响应 error「daemon 未支持压缩，请升级 daemon」；daemon 离线 → DaemonRuntimeOffline「daemon 离线」；15s 无响应 → DaemonRpcTimeout「daemon 未响应压缩命令」（send_rpc 异常三态齐备 ws_hub.py:502-560）。
