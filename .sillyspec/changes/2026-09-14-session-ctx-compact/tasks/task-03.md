...
implementation:
  ...
  - 'NEW session-manager/compact.ts——照 turn-control.ts:157 三态守卫先例落新子模块：compact(mgr, sessionId) 六守卫——① store 无该 session → SessionNotFoundError ② status=running 拒绝 ③ status=reconnecting 拒绝 ④ status=ended/failed 拒绝（SessionNotActiveError 形态）⑤ driver 未实现 compact 方法 → 拒绝 error ⑥ getProviderCaps(provider).compact=false → 拒绝 error；全通过 → driver.compact(handle) 透传 CompactResult；守卫拒绝 throw 使 RPC handler 上抛 → backend 收 DaemonRpcRemoteError 映射结构化 error'
