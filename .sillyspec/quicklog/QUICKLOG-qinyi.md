...
## ql-20260622-003-7f3a | 2026-06-22 23:34:48 | 修复 agent-log-viewer 2 个 P1 性能问题（useMemo + tool duration 迁移到 normalize）+ P3 注释对齐
状态：已完成
结果：P1-1 给 processedLogs/visibleLogs/filteredLogs/turns 全部加 useMemo（依赖 logs 引用 + activeFilters，turns 依赖 filteredLogs 引用）。P1-2 删 render 期 computeToolDurationMs（原 N×M 回查 allLogs），耗时迁移到 normalize：mergeToolResult 加 sourceLog 参数，配对 [TOOL_RESULT] 时用 diffLogMs 预算 toolDurationMs 存入 ProcessedLog（sourceLog!==target.log 守卫保持自合并 undefined 语义、首次设置取首条 result）；4 个调用点传 current.log；render 改读 processedLog.toolDurationMs。P3-1 normalize.ts 注释「多出 ≥4 字符」对齐为代码实际的「≥8」（两处）。验证：vitest 47/47 全绿（normalize 35 + agent-log-viewer 12，含 5.7s 耗时 + 进行中不显示秒数用例）；ESLint 0 errors（2 warning 是 onPermissionResolved 类型 pre-existing 非本次）。allLogs 参数仍被 isPendingReplied 使用未变 unused。
文件：
- frontend/src/components/agent-log-viewer.tsx (P1-1 useMemo + P1-2 删 computeToolDurationMs 改读 toolDurationMs)
- frontend/src/components/agent-log/normalize.ts (P1-2 mergeToolResult 配对预算 toolDurationMs + P3-1 注释 ≥4→≥8)
- frontend/src/components/agent-log/types.ts (P1-2 ProcessedLog 加 toolDurationMs 字段)
