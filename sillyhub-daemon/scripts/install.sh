...
EOF
  ...
  # node.exe 所在目录转成 Windows 路径（C:\...），给 .cmd 写绝对路径兜底（不依赖运行时 PATH 含 node）
  local win_node_dir_win=""
  if [[ "$NODE_BIN" == *.exe ]]; then
    local node_dir
    node_dir="$(dirname "$NODE_BIN")"
    if [[ "$IS_WSL" -eq 1 ]]; then
      win_node_dir_win="$(wslpath -w "$node_dir" 2>/dev/null || echo "")"
    else
      # Git Bash: /c/nvm4w/nodejs → C:\nvm4w\nodejs
      win_node_dir_win="$(echo "$node_dir" | sed 's|^/\([a-zA-Z]\)/|\1:|; s|/|\\|g')"
    fi
  fi

  # bundle 路径用 %~dp0 自相对（.cmd 所在目录），故 win_bin_dir 仅用于判断能否确定 Windows bin 目录
  local win_bin_dir=""
  if [[ "$IS_WSL" -eq 1 ]]; then
...
if exist "${win_node_dir_win}\node.exe" (
  "${win_node_dir_win}\node.exe" "%~dp0${BUNDLE_NAME}" %*
) else (
  node "%~dp0${BUNDLE_NAME}" %*
)
CMDEOF
    # Windows .cmd 必须 CRLF 换行：heredoc 默认 LF，LF + UTF-8 中文 REM 在 GBK cmd.exe 下
    # 会让注释行被当命令执行（实测）。统一转 CRLF（awk 跨 Git Bash/WSL/Linux 行为一致）。
    awk 'BEGIN{ORS="\r\n"} {sub(/\r$/,""); print}' "$cmd_wrapper" > "$cmd_wrapper.tmp" && mv "$cmd_wrapper.tmp" "$cmd_wrapper"
