...
describe('protocol — batch 协议常量值不回归（FR-09 / AC-08）', () => {
  ...
  it('MSG 总数 = 19（10 旧 + 5 session/permission + 1 SESSION_RESUME + 1 SELF_UPDATE + 1 LEASE_CANCEL + 1 PROVIDER_CONFIG_CHANGED），互不干扰', () => {
    expect(Object.keys(MSG)).toHaveLength(19);
  });
  ...
  it('LEASE_CANCEL（task-04 batch 即时取消）字符串值 = daemon:lease_cancel（与 backend DAEMON_MSG_LEASE_CANCEL 对齐）', () => {
    expect(MSG.LEASE_CANCEL).toBe('daemon:lease_cancel');
    expect(MSG.LEASE_CANCEL.startsWith('daemon:')).toBe(true);
  });
