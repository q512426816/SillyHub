...
function baseInput(sessionId: string, runId: string) {
  return {
    ...
    // scan 真阻塞（per-session）：manualApproval=true 才注入 canUseTool。
    manualApproval: true,
  };
}
...
describe('pending 审批时 interrupt → cancelAllPending（AC-09.3 / 边界 3）', () => {
  it('interrupt 触发 resolver.abortAll("session_interrupted")；pending 回调 deny；pendingCount 归零', async () => {
    ...
    const pending = canUseTool('AskUserQuestion', { questions: [{ question: 'q' }] });
  ...
  it('interrupt 后 SDK 产 result subtype=error_during_execution → onResult 标 run failed，session 回 active', async () => {
    ...
    await sm.create(baseInput('s1', 'r1'));
    // pending 审批在途。
    void d.capturedOptions!.canUseTool!('AskUserQuestion', { questions: [{ question: 'q' }] });
...
describe('pending 审批多并发 + 乱序到达（AC-09.6 / 边界 8）', () => {
  it('同 turn 多 pending（多工具并发审批）各自独立 resolve；cancelAllPending 一次性 reject 全部', async () => {
    ...
    await sm.create(baseInput('s1', 'r1'));
    const canUseTool = d.capturedOptions!.canUseTool!;
    // scan 真阻塞：AskUserQuestion 分流——只有 AskUserQuestion 走 register；
    // 3 次调用各自独立 requestId，pendingCount=3（验证多 pending 清理）。
    const p1 = canUseTool('AskUserQuestion', { questions: [{ question: 'q1' }] });
    const p2 = canUseTool('AskUserQuestion', { questions: [{ question: 'q2' }] });
    const p3 = canUseTool('AskUserQuestion', { questions: [{ question: 'q3' }] });
