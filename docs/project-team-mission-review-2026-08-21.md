...
| BE-P1-6 | P1 | 主 run `pending + no_online_daemon` 无重派机制（注释承诺"靠 reconcile 重派"但不存在）；叠加项目维度 mission 的 run 完成回调被 `change_id is None` 短路，schedule_loop 对项目 mission 无触发点 → mission 挂死只能重启 | `backend/app/modules/agent/orchestrator.py:321-332`、`backend/app/modules/daemon/run_sync/service/__init__.py` |
