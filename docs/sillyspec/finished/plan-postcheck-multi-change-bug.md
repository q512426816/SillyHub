...
## 修复状态（2026-07-01，CLI v3.20.5）
CLI 已升级至 3.20.5，resolveChangeDir 相关崩溃已修复。（调用即 TypeError）亦已修复。❌ 用法: sillyspec progress complete-stage <stage> 绕过法不再需要。

## 修复状态（2026-07-01，CLI v3.20.5）
CLI 已升级至 3.20.5。resolveChangeDir 相关崩溃已修复（含 resolveChangeDir is not a function TypeError 变体）。sillyspec progress complete-stage 绕过法不再需要。
