# worktree doctor --fix 会把「活跃变更的分支」当孤儿删除（元数据竞态）— 已修复

- 发现日期：2026-08-20（变更 2026-08-20-runtime-readpoint-repo-first execute Step12）
- 状态：已修复（工具仓 commit `f191fc4`，随 3.26.13 发布）：删分支前交叉核对进度库活跃变更注册表——分支名 ∈ 活跃变更 → 保留（active-branch，只报告）；进度库存在但读失败 → 保守不自动删；仅「无 meta 且非活跃」才删。采用建议的多源交叉核对（以进度库为权威源），「最近 10 分钟 commit 不删」时间启发式未单独实现（活跃注册表核对已覆盖该竞态根因）。测试 `worktree-doctor-active-branch.test.mjs`（本机 2026-08-21 实跑通过）
