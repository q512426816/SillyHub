...
**证据链**：
- 前端 `frontend/src/lib/agent.ts:250` `killAgentRun` → POST `/workspaces/{wid}/agent/runs/{rid}/kill`
- `backend/app/modules/agent/router.py:432` `kill_agent_run`
- `backend/app/modules/agent/service.py:822` `kill_run` —— **唯一动作是 `cancel_lease(run_id)`，不改 AgentRun.status、不发 WS**
...
- 心跳循环 `sillyhub-daemon/src/task-runner/spawn-stream.ts:92` `_runLeaseHeartbeatLoop` 只在 batch `runLease`（sillyhub-daemon/src/task-runner/spawn-stream.ts:92）内启动 → **interactive session 没有任何机制感知 backend 的 cancel**
...
- 创建：`backend/app/modules/agent/router.py` `create_mission` → GLM 规划 → 建 Worker Run → 治理门 → `dispatch_worker` 派发
- 派发：`backend/app/modules/agent/execution.py:80` `dispatch_worker` → `backend/app/modules/agent/placement.py:92` `dispatch_to_daemon`（batch lease）
- 回灌：`backend/app/modules/daemon/lease/service.py:313` `complete_lease` 把 daemon 上报 output 写进 `AgentRun.output_redacted`
- 收敛入口：`backend/app/modules/daemon/lease/service.py:717` 调 `converge_mission_for_completed_run`（`backend/app/modules/agent/finalizer.py:230`）
- artifact 回灌：`backend/app/modules/agent/finalizer.py:231` 调 `collect_completed_artifacts`（`backend/app/modules/agent/execution.py:830`）
- GLM 合并：全 worker 终态（`derive_status` 返回 done/degraded）时 `backend/app/modules/agent/finalizer.py:182` 调 `finalize_bootstrap_mission` → `_glm_merge`，失败回退 `_concat_merge`
- 前端可读：`frontend/src/components/mission-summary-card.tsx`（旧 mission-console.tsx 已重构） 遍历 worker artifacts 渲染
- **断点（体验，非功能）**：合并 artifact 挂在 mission 第一个 worker（`backend/app/modules/agent/finalizer.py:119` `_carrier_run`），用户认不出"这是最终结果"
...
- `budget_tokens` 字段（`backend/app/modules/agent/model.py:1850`）**全代码无任何强制点**
...
**关联 bug**：`MissionControl.cancel`（`backend/app/modules/agent/control.py:124`）cancel mission 时**只改 AgentRun.status，不调 `cancel_lease`** → 又造僵尸（与发现 1 同病）。daemon 不会被通知，worker 继续跑。
...
- **要不要 SESSION_END 替代 INTERRUPT**：INTERRUPT 是 turn 级（session 存活），SESSION_END 才真杀进程（`sillyhub-daemon/src/interactive/codex-app-server-driver.ts:1480` close → SIGTERM）。用户语义"终止 run"建议 INTERRUPT 足够（保留 session 上下文）；若要"彻底停烧 token"则用 END——**需设计决策**

#### P0-2 修 MissionControl.cancel 造僵尸
- **改动**：`backend/app/modules/agent/control.py:350` 对每个 active worker，除改 status，补调 `DaemonLeaseService(session).cancel_lease(run.id)`
...
- **改动**：
  1. `backend/app/modules/spec_workspace/router.py:331` `bootstrap_spec_workspace` 透传 `mode` 参数（当前硬编码 single）
  2. 前端 missions 页或工作区初始化加"用团队模式分析"入口
  3. 端到端真跑一次验证（ROADMAP 列的"delegate_task spike 运行时验证"）
- **依据**：只读 mission 链路端到端打通（发现 3）；`route()` 三档 single/team/auto 已实现（`backend/app/modules/agent/delegation.py:39-53`）；team 关键词触发（`_TEAM_HINT_KEYWORDS`：扫描/架构/多模块/重构/...）
- **改动量**：小（参数透传 + 前端一个按钮）
- **风险**：低
- **价值**：立刻让你"真正用上 agent 团队"做并行分析

#### P1-2 前端补 resume 按钮
- **改动**：`frontend/src/lib/agent.ts` 加 `resumeRun`（后端 `resume_agent_run` API 已有，sillyhub-daemon/src/api-types.ts:2876）；智能体控制台失败/中断 run 加"续跑"按钮
...
- **改动**：`frontend/src/app/(dashboard)/workspaces/[id]/approvals/page.tsx` 占位 button 接详情抽屉
- **改动量**：小

#### P1-5 mission 最终结果独立展示
- **改动**：`frontend/src/components/mission-console.tsx` 加"最终合并结果"独立区（现在合并 artifact 混在第一个 worker 里，`_carrier_run` `backend/app/modules/agent/finalizer.py:119`）
- **改动量**：小
...
- **改动**：
  ...
  2. 可挂 `cleanup_stale_runs`（`backend/app/modules/agent/service.py:1418 / 1753`）同类后台任务附近
...
- **改动（4 处）**：
  1. **per-worker worktree 隔离（硬阻塞，最关键）**：`backend/app/modules/agent/execution.py:49` `dispatch_worker` 给每个写 worker 起 `git worktree add` 临时分支目录，不再共享 `ws.root_path`
  2. `collect_completed_artifacts`（`backend/app/modules/agent/execution.py:199`）把 daemon 上报的 `result["patch"]` 存成 `AgentArtifact(kind="patch")`
  3. `converge_mission_for_completed_run`（`backend/app/modules/agent/finalizer.py:230`）按 mission 类型分流，execute 走 `finalize_execute_mission`
  4. `finalize_execute_mission`（`backend/app/modules/agent/finalizer.py:244`）实现真正的 patch 合并（顺序 apply + 冲突裁决，复用 `_apply_patch_to_worktree` 引擎 `backend/app/modules/daemon/lease/service.py:370`）
- **依据**：采集（`sillyhub-daemon/src/task-runner.ts:691`）/ 上报（`sillyhub-daemon/src/daemon.ts:3322`）/ 单 agent apply（`backend/app/modules/daemon/lease/service.py:481`）全已就位，缺的就是隔离 + 合并
...
- **改动量**：小
- **关联**：Finalizer 同样走 GLM（`backend/app/modules/agent/finalizer.py:94-120`），一起配置化
...
| # | 动作 | 依据 | 改动量 |
|---|---|---|---|
| P3-1 | 失败 run 自动建 incident 工单 | incident 模块（`backend/app/modules/incident/service.py:47-77`）纯手工 CRUD，与 agent 失败零联动 | 中 |
| P3-2 | tool 失败率前端可见 | 后端算了（`backend/app/modules/agent/service.py:64-223`），注释明说 non-blocking/no alert/no display | 中 |
| P3-3 | agent 团队拓扑图 | mission-summary-card 纯列表（`frontend/src/components/mission-summary-card.tsx`），看不出 worker 依赖/时序 | 中 |
| P3-4 | 更多 interactive driver | 探测 12 provider（`sillyhub-daemon/src/agent-detector.ts:104`），driver 仅 claude+codex（`sillyhub-daemon/src/cli.ts:603`） | 大 |
| P3-5 | 启用 reviewer/qa/component_lead 系统角色 | auth 种子 7 角色，工作区成员 API 只让授 owner/developer/viewer（`backend/app/modules/workspace/members_service.py:41`） | 小 |
...
- **interactive driver 仅 2 个**：claude（`claude-sdk-driver.ts`）+ codex（`codex-app-server-driver.ts`），注册于 `sillyhub-daemon/src/cli.ts:603`
...
- 命名归一化：backend 用 `agent_type='claude_code'`，daemon detector 用 `'claude'`，必须 `normalizeProvider()`（`sillyhub-daemon/src/agent-detector.ts:202-207`）

### 5.2 任务类型（dispatch 入口 × lease kind）
| 类型 | 入口 | lease kind |
|---|---|---|
| 扫描 scan | `backend/app/modules/agent/service.py:1711` `start_scan_dispatch` | interactive（manual_approval+ask_user_only，`backend/app/modules/agent/placement.py`） |
| 变更流程 stage | `backend/app/modules/agent/service.py:1430` `start_stage_dispatch` | interactive |
| 任务执行 task | `backend/app/modules/agent/service.py:403` `start_run` | batch（task_id+lease_id） |
| 对话 dialog | `backend/app/modules/daemon/session/service.py` `create_session` | interactive（长生命周期） |
| 初始化 init | `backend/app/modules/agent/service.py:2009` `start_init_dispatch` | batch（mode='init'，不走 agent 进程） |
| 多 agent mission | `backend/app/modules/agent/router.py` `create_mission` | batch（dispatch_to_daemon） |
...
- `AgentMission`（`backend/app/modules/agent/model.py:1754`）：**status 不持久化**，由 derive_status 派生（`backend/app/modules/agent/mission.py:29-54`）；无 final/merged 字段
- `AgentArtifact`（`backend/app/modules/agent/model.py:1937`）：kind ∈ summary/patch/test_result/evidence；content_ref 截断
- `AgentRunDependency`（`backend/app/modules/agent/model.py:1902`）：DAG 边（v1 flat，无独立 wiring）

### 5.4 管理与协作能力（成熟，列出备查）
- **双层审批**：
  - 工具级：`permission_service.py` canUseTool（5min 超时 deny）+ AskUserQuestion（持久化 session_dialog_requests，不超时）
  - 阶段级：PendingReview 四面板（proposal/plan/human_test/archive，`backend/app/modules/change/service.py:1199/1257/1329/1584`）
- **Gate 三态决策**（`backend/app/modules/change/dispatch.py:312-443`，P3 driver-gate）：exit 0 推进 / exit 1 重跑（连续 ≥3 次升级 exit 2）/ exit 2 fail-loud 卡住
- **权限**：7 角色（platform_admin/workspace_owner/component_lead/developer/reviewer/qa/viewer）+ ~40 权限点（`backend/app/modules/auth/permissions.py:34-144`），成员 API 仅授 owner/developer/viewer
- **daemon 路由**：per-member binding（WorkspaceMemberRuntime，`backend/app/modules/workspace/member_runtimes/model.py`），无 binding 即 `NoOnlineDaemonError`（`backend/app/modules/agent/placement.py:691-808`）
- **Coordinator 用 GLM**：直接 messages API 不走 CLI（`backend/app/modules/agent/delegation.py:150-202`，spike 04 结论：CLI 的 agentic system prompt 让 GLM 拒绝输出纯委派 JSON）
...
| 注释声明但行为已验证 | 注释与实际不符（如 `backend/app/modules/agent/mission.py:6-7` 旧注释 vs execution.py 已实现） | 标注"注释过时" |
