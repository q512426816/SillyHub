...
- `budget_tokens` 字段（`backend/app/modules/agent/model.py:1917`）**全代码无任何强制点**
...
- `AgentMission`（`backend/app/modules/agent/model.py:1821`）：**status 不持久化**，由 derive_status 派生（`backend/app/modules/agent/mission.py:29-54`）；无 final/merged 字段
- `AgentArtifact`（`backend/app/modules/agent/model.py:2004`）：kind ∈ summary/patch/test_result/evidence；content_ref 截断
- `AgentRunDependency`（`backend/app/modules/agent/model.py:1969`）：DAG 边（v1 flat，无独立 wiring）
