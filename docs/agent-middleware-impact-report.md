...
| `AgentSession` | `backend/app/modules/agent/model.py:606` | 一个交互式会话（跨多个 AgentRun turn 的 1:N 容器） |
| `AgentMission` | `backend/app/modules/agent/model.py:917` | 多 agent 协同的聚合根（多个 AgentRun 的父容器） |
...
| `agent_sessions` | `backend/app/modules/agent/model.py:626` | `provider VARCHAR(30)` | 新增 `agent_profile_id UUID FK` |
| `agent_missions` | `backend/app/modules/agent/model.py:719` | `main_agent_config JSON`, `worker_preset JSON` | worker_preset 条目新增 `agent_profile_id` 替代 inline agent_type |
...
4. **Mission worker_preset 和 AgentProfile 的关系**
   - `backend/app/modules/agent/model.py:1875`：`worker_preset` 是 `list[dict]` JSON，每条含 `{agent_type, model, objective, role}`
