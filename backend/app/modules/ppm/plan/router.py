...
# 列顺序对齐新建明细弹窗(page.tsx 开立信息块)+状态置序号后+执行状态置执行人后
# (ql-20260727-005 补序号+要求;ql-20260727-006 按弹窗顺序调列序)。
_MILESTONE_DETAIL_GROUP_COLUMNS = [
    ColumnDef(field="no", header="序号", width=8),
    ...
    ColumnDef(field="requirements", header="要求", width=30),
...
def _build_weekly_plan_workbook(rows: list[dict[str, Any]]) -> bytes:
    """加载用户提供的模板 xlsx,从第 6 行开始填数据,保留模板原始表头/样式/合并。

    模板位置:backend/templates/weekly-plan-template.xlsx
    数据行从行 6 开始,B 列(项目名称)同名合并、E 列(平台/子系统)同平台合并。
    """
    ...
    for idx, r in enumerate(rows):
        ...
        for c, v in enumerate(values, 1):
            ...
            cell.font = data_font
            cell.border = border
            cell.alignment = left_align if c in (2, 5, 6, 7) else center
