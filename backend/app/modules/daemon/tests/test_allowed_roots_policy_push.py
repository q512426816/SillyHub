...
async def test_put_allowed_roots_pushes_policy_update(
    ...
    body = resp.json()
    assert body["allowed_roots"] == new_roots

    # send_policy_update invoked exactly once with (rid, new_roots, version>=1).
    hub.send_policy_update.assert_awaited_once()
    call_args = hub.send_policy_update.await_args.args
    assert call_args[0] == rt.id
    assert call_args[1] == new_roots
    version = call_args[2]
    assert isinstance(version, int)
    assert version >= 1

    # DB actually persisted.
    await db_session.refresh(rt)
    assert rt.allowed_roots == new_roots
...
async def test_put_allowed_roots_offline_runtime_still_returns_200(
    ...
    # Case A: send returns False (runtime offline / send failed).
    ...
    assert resp.status_code == 200, resp.text
    assert resp.json()["allowed_roots"] == new_roots
    ...
    assert resp2.json()["allowed_roots"] == ["/srv/other"]
...
async def test_put_allowed_roots_version_monotonic_across_writes(
    ...
    async def _capture(*args: object) -> bool:
