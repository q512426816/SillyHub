...
覆盖 design §6 五边界（2026-07-10-remove-server-local-workspace-mode 删 Step 2
legacy fallback 后）：
- not_bound：无 binding 行（不再回退 workspace.daemon_runtime_id）。
...
async def test_resolve_no_binding_no_legacy_raises_not_bound(db_session) -> None:
    """无 binding 行 → not_bound（不再回退 workspace.daemon_runtime_id）。"""
