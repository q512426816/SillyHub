...
* ``relations[].target`` references an
  unknown workspace                       -> relation dropped, warn
...
class ParsedWorkspace:
    """One workspace derived from a single ``projects/*.yaml`` file."""
...
class ParsedRelation:
    """One edge between two workspaces, by component_key (UUIDs come later)."""
...
@dataclass(slots=True)
class ParseResult:
    """Result of parsing all ``projects/*.yaml`` files."""
...
class WorkspaceParser:
    ...
    def parse(self, workspace_root: str | Path) -> ParseResult:
        """Parse all YAML files in the projects sub-directory.

        Args:
            workspace_root: Absolute path to the workspace root directory.

        Returns:
            ParseResult containing workspaces, relations, warnings, and errors.

        This method is a pure function with no side effects.
        """
        result = ParseResult()
        ...
        for yaml_path in sorted(projects_dir.glob("*.yaml")) + sorted(projects_dir.glob("*.yml")):
            ...
            if not isinstance(component_key, str) or not component_key.strip():
                # Fallback: use 'name' field or filename stem as component key
                fallback = raw.get("name") or rel.stem
                if isinstance(fallback, str) and fallback.strip():
                    component_key = fallback.strip()
                    result.warnings.append(
                        ParseIssue(
                            code="missing_id",
                            file=rel,
                            detail=f"No 'id' field; using '{component_key}' as component key",
                            severity="warning",
                        )
                    )
                else:
            ...
            workspace = self._parse_workspace(component_key, raw, root, rel)
