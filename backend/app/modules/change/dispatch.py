...
async def reconcile_stale_runs(
    ...
    """Mark stale ``running`` AgentRuns as killed.
    ...
    Marking such runs ``killed`` releases the ``has_active_run`` guard so
    explicit stage dispatch (``advance_change_stage``) is no longer blocked.

    Stage progression is NOT auto-recovered here — the caller / user must
    re-trigger dispatch explicitly (形态A 按需触发, design §4.1 ⑤ / D-007).
...
def merge_gate_results(worker_results: list[dict]) -> dict:
    ...
    形态A（按需触发）：三态决策（exit 0 推进 / 1 打回 / 2 卡住）改由
    advance_change_stage MCP tool 显式触发时读取，不再自动连轴。
