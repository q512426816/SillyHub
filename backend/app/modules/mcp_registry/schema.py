...
脱敏规则（R-05）：env 键名含 token/key/secret/password 子串（大小写不敏感）视为
secret——沿用 ``settings/router.py:164`` ``_SECRET_KEY_MARKERS`` 同款四标记规则，
与旧 ``_redact_mcp_env`` 双向一致；脱敏在 DTO 构造期由 ``field_validator`` 强制
执行（含 from_attributes 直吃 ORM 行），service/router 无法意外漏脱。
...
class McpServerUpdate(BaseModel):
    ...
    # env 中 secret 键由 service 重抽列重加密（task-02）。
...
class McpServerRead(BaseModel):
    """列表项（env 脱敏 + 绑定态 + 诊断徽标，design REST 列表行）。
...
class McpTemplateRead(BaseModel):
    """模板列表项（明文本无 secret，输出仍走脱敏兜底——防御纵深）。"""
