...
class Incident(BaseModel, table=True):
    ...
    workspace_id: uuid.UUID = Field(sa_column=Column(Uuid, ForeignKey("workspaces.id"), nullable=False))
    ...
    resolved_by: uuid.UUID | None = Field(default=None, sa_column=Column(Uuid, ForeignKey("users.id"), nullable=True))
    release_id: uuid.UUID | None = Field(default=None, sa_column=Column(Uuid, ForeignKey("releases.id"), nullable=True))
...
class Postmortem(BaseModel, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, sa_column=Column(Uuid, primary_key=True))
    incident_id: uuid.UUID = Field(sa_column=Column(Uuid, ForeignKey("incidents.id"), nullable=False, unique=True))
