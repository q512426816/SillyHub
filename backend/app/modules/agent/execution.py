...
def render_worker_prompt(run: AgentRun, *, mode: str = "git") -> str:
    ...
    if mode == "direct":
        return (
            ...
            "【收尾消息（必须遵守）】你的最后一条消息必须是给主控的完整结论文本"
            "（含产出文件路径、关键发现摘要）——它会被原样存为 summary 产物供主控"
            "读取。严禁以动作语句结尾（如「现在我去提交」）——那句话会成为主控"
            "看到的全部产出，真实结论反而丢失。\n\n"
            "【直通工作区约束（必须遵守）】\n"
    ...
    return (
        ...
        "供 Coordinator 收敛。不要输出与目标无关的内容。\n"
        "【结果落盘（必须遵守）】把任务产出的正文内容（清单/报告/结论等）"
        "写入一份文件（如 results.md 或与目标匹配的产物文件）并随 commit 提交"
        "——主 agent 通过 get_worker_result 只能取到落盘产物（artifact），"
        "只写在对话里的结果它拿不到。\n"
        "【收尾消息（必须遵守）】你的最后一条消息必须是给主控的完整结论文本"
        "（含产出文件路径、commit 号、关键发现摘要）——它会被原样存为 summary "
        "产物供主控读取。严禁以动作语句结尾（如「现在我去提交」）——先提交、"
        "确认成功后，再输出完整结论作为最后一条消息。\n\n"
        "【worktree 协作约束（必须遵守）】\n"
        ...
        "没有 commit 就没有可合并的产物。提交后以完整结论文本收尾（见收尾消息约束）。\n"
