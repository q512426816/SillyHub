...
验收返工：QA 发现 NOT NULL+FK 会压断 4 个无会话存量创建入口，改 nullable）：
1. agent_missions 加 session_id（uuid FK agent_sessions.id，nullable，普通索引
   ix_agent_missions_session_id）——会话内团队的 mission 绑定发起会话；external
   mission（change 执行链/GLM fallback/扫描引导）为 NULL，非 NULL 即绑定会话；
2. 活跃态部分唯一索引 uq_agent_missions_session_active（WHERE session_id IS
   NOT NULL AND converged_at IS NULL AND cancelled_at IS NULL）——一个会话同时
   至多一个未收敛未取消的 mission（R-07 单活跃约束 / Grill NEW-3 并发守卫的
...
_PARTIAL_WHERE = "session_id IS NOT NULL AND converged_at IS NULL AND cancelled_at IS NULL"


def upgrade() -> None:
    # 1. session_id 列：会话 mission 绑定发起会话；external mission 为 NULL
    op.add_column(
        "agent_missions",
