 ...
 * 六 API——端点由 task-02/03 落地（backend/app/modules/daemon/router.py），照
 * 上方 archive/unarchive/ctx-window 模板（apiFetch + encodeURIComponent）。
 * 类型经 gen:types 从 openapi.json 生成（components["schemas"] 引用，禁止手写）；
 * 错误不在本 client 本地处理，统一由 apiFetch 抛 ApiError（401 refresh +
 * 403/404/409/422 业务码透传）。
 */
