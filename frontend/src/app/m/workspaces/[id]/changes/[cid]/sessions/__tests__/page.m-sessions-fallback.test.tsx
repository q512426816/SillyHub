/**
 * task-10 · changes/[cid]/sessions 深链兜底 redirect 薄壳测试
...
import Page from "@/app/m/workspaces/[id]/changes/[cid]/sessions/page";
...
describe("m/workspaces/[id]/changes/[cid]/sessions 深链兜底", () => {
  it("挂载即 replace 到 /m/workspaces/w1/sessions（恰一次）", async () => {
    render(<Page params={{ id: "w1", cid: "c1" }} />);
    await waitFor(() => {
  ...
  it("渲染 null（零 UI）", () => {
    const { container } = render(<Page params={{ id: "w1", cid: "c1" }} />);
