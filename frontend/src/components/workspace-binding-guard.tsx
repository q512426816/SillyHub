 ...
 *
 * - 未绑定（unbound）：return null（门禁后移，配置引导交给概览页 WorkspaceConfigCard）。
 * - 已绑定（bound）：渲染「编辑我的接入配置」入口按钮，
...
export function WorkspaceBindingGuard({ workspaceId }: Props) {
  ...
  // 2026-07-26-ungate-workspace-entry / FR-02 / D-004：门禁后移，未绑定不再渲染绑定表单阻断，
  // 一律 return null（概览页 WorkspaceConfigCard 接管可选配置引导）。已绑定保留编辑入口。
