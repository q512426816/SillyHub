 ...
 * 图标映射与单行树容器已抽离共享组件（ql-20260821-013-2c1a）：FileNodeIcon
 * （components/ui/file-node-icon，按扩展名分型配色，开发语言细分见该文件）+ TreeBox
 * （components/ui/tree-box，单行覆盖+横向滚动）。
...
function entriesToNodes(entries: ExplorerEntry[], parentPath: string): DataNode[] {
  return sortEntries(entries).map((entry) => ({
    ...
    icon: <FileNodeIcon name={entry.name} type={entry.type} />,
  }));
}
...
export function FileExplorer({ workspaceId, onSelectFile }: FileExplorerProps) {
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col gap-2">
                ...
                <ul className="max-h-64 overflow-auto py-1">
                  {(results ?? []).map((match) => (
                    <li key={match.path}>
                      ...
                      >
                        <FileNodeIcon name={match.name} type={match.type} size="h-3.5 w-3.5" />
      ...
      {/* 文件树：根加载中 / 空态 / 树。单行展示 + 16px 缩进 + 横向滚动语义
          统一在 TreeBox（components/ui/tree-box，ql-20260821-013 抽离）。 */}
      {rootLoading ? (
        ...
        <TreeBox className="min-h-0 flex-1 rounded border">
          <Tree
            treeData={treeData}
            loadData={onLoadData}
